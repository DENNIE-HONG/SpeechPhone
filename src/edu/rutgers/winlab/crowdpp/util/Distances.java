...
public class Distances {
	...
	public static double Cosine(SimpleMatrix a, SimpleMatrix b) {
		...
		double []a_MFCC = new double[a.numCols()];
		double []b_MFCC = new double[b.numCols()];
		a_MFCC=Maths.getColMean(a_norm);
		b_MFCC=Maths.getColMean(b_norm);
		double deg = Math.toDegrees(Math.acos(Maths.dotProduct(a_MFCC, b_MFCC) / (Maths.getNorm2(a_MFCC) * Maths.getNorm2(b_MFCC))));
		return deg;
	}
	
	/** @return the newcosine distance */
	public static double newCosine(SimpleMatrix a, SimpleMatrix b, double c, double d) {
		double [][]a_norm = new double[a.numRows()][a.numCols()];
		for(int i = 0; i < a.numRows(); i++) {
			for(int j = 0; j < a.numCols(); j++) {
				a_norm[i][j] = a.get(i,j);
		...
		a_MFCC=Maths.getColMean(a_norm);
		b_MFCC=Maths.getColMean(b_norm);
//		a_MFCC = Maths.getMFCCMean(a_norm);
//		b_MFCC = Maths.getMFCCMean(b_norm);
		double []a_MFCCF0 = new double[a.numCols()+1];
		double []b_MFCCF0 = new double[b.numCols()+1];
		for (int i = 0; i < a.numCols()+1; i++){
			if( i < Constants.mfccnum)
			{
				a_MFCCF0[i] = a_MFCC[i];
				b_MFCCF0[i] = b_MFCC[i];
			}
		...
		double deg = Math.toDegrees(Math.acos(Maths.dotProduct(a_MFCCF0, b_MFCCF0) / (Maths.getNorm2(a_MFCCF0) * Maths.getNorm2(b_MFCCF0))));
		return deg;
	}
